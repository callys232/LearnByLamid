...
export default async function CertificatePrintPage({ params }: PageProps) {
  ...
  const cert = mockCertificates.find(
    (c) => c.id === id || c.verificationId === id,
  );
  if (!cert) notFound();

  const user = mockUsers.find((u) => u.id === cert.userId);
  const course = cert.courseId
    ? mockCourses.find((c) => c.id === cert.courseId)
    : null;
  const program = cert.programId
    ? mockPrograms.find((p) => p.id === cert.programId)
    : null;
  const title = program?.title ?? course?.title ?? cert.title;

  // Prefer NEXTAUTH_URL or NEXT_PUBLIC_APP_URL for runtime absolute URLs.
  // If neither is set (e.g. in some dev environments), fall back to a relative path
  // so we don't accidentally produce a localhost link on a live site.
  const baseUrl = (
  ...
  const verifyUrl = baseUrl
    ? `${baseUrl}/verify/${cert.verificationId}`
    : `/verify/${cert.verificationId}`;
      ...
      >
        ...
        >
          ...
          {[
            ...
            <div
              key={pos}
              className={`absolute ${pos} h-6 w-6 border-2`}
              style={{ borderColor: "#C12129" }}
            />
          ))}
          ...
          <div className="mb-6 flex flex-col items-center gap-1">
            <div
              className="flex h-14 w-14 items-center justify-center rounded-xl mb-2"
              style={{ background: "#C12129" }}
            >
          ...
          >
            CERTIFICATE OF{" "}
            {cert.level === "professional" ? "ACHIEVEMENT" : "COMPLETION"}
          </p>
